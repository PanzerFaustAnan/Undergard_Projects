   ...
   <form action="" method="post">
      ...
      <h3>filter your search</h3>
         ...
         <div class="flex">
            ...
            <div class="box">
               <p>how many BWV(Bed,Washroom,Varenda)</p>
               <select name="BWV" class="input" required>
                  <option value="1">1 BWV</option>
                  <option value="2">2 BWV</option>
                  <option value="3">3 BWV</option>
                  <option value="4">4 BWV</option>
                  <option value="5">5 BWV</option>
                  <option value="6">6 BWV</option>
               </select>
            </div>
            <div class="box">
               <p>maximum budget</p>
...
if(isset($_POST['h_search'])){
   ...
   $BWV = $_POST['BWV'];
   $BWV = filter_var($BWV, FILTER_SANITIZE_STRING);
   $min = $_POST['min'];
   ...
   $select_properties = $conn->prepare("SELECT * FROM `property` WHERE address LIKE '%{$location}%' AND type LIKE '%{$type}%' AND offer LIKE '%{$offer}%' AND BWV LIKE '%{$BWV}%' AND status LIKE '%{$status}%' AND furnished LIKE '%{$furnished}%' AND price BETWEEN $min AND $max ORDER BY date DESC");
   $select_properties->execute();
      ...
      <form action="" method="POST">
         ...
         <div class="box">
            ...
            <div class="flex">
               ...
               <p><i class="fas fa-bed"></i><span><?= $fetch_property['BWV']; ?> BWV</span></p>
